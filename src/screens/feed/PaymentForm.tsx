...
const PaymentForm: React.FC<Props> = ({route, navigation, setLoading}: any) => {
  ...
  const handlePaymentFromAccount = () => {
    ...
    try {
      ...
      addDoc(collection(db, 'donations'), save_donation_object);

      // Add project to users donation list
      const _userRef = doc(db, 'users', id);
      updateDoc(_userRef, {
      ...
      updateDoc(doc(db, 'fundraising', id), {
        collect: Number(collect) + Number(value),
        donation: arrayUnion(save_donation_object),
      });
